...
struct SEG
{
  ...
  int build(vector<int> &a , int x= 1, int lx = 1 , int rx = seg_size){
    if(lx==rx){ return tree[x] = (lx%2 ? a[lx]  : -a[lx]);}
    return tree[x] = merge(build(a , lft) , build(a , rit));
  }
  int set(int i, int v , int x= 1 ,int lx = 1 ,int rx = seg_size){
    if(lx==rx){
      return tree[x] = lx%2 ? v  : -v;
    }
    int mid = (lx+rx)/2;
